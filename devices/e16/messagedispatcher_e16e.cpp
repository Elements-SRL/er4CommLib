...
void MessageDispatcher_e16e_Artix7_PCBV01_V01::initializeDevice() {
...
bool MessageDispatcher_e16e_Artix7_PCBV01_V01::checkProtocolValidity(string &message) {
    bool validFlag = true;
...
void MessageDispatcher_e16e_Artix7_PCBV01_V01::setFerdParameters() {
...
ErrorCodes_t MessageDispatcher_e16e_Artix7_PCBV01_V01::updateVoltageOffsetCompensations(vector <Measurement_t> &offsets) {
...
MessageDispatcher_e16e_Artix7_PCBV01_fwV01::MessageDispatcher_e16e_Artix7_PCBV01_fwV01(string id):
    MessageDispatcher(id) {
    ...
    txStatus[txStatusIdx++] = 0x7f; // CFG1
    txStatus[txStatusIdx++] = 0x7f; // CFG2
    txStatus[txStatusIdx++] = 0x03; // CFG3
    txStatus[txStatusIdx++] = 0x7f; // CFG4
    txStatus[txStatusIdx++] = 0x7f; // CFG5
...
void MessageDispatcher_e16e_Artix7_PCBV01_fwV01::initializeDevice() {
    this->setSamplingRate(defaultSamplingRateIdx, false);
...
bool MessageDispatcher_e16e_Artix7_PCBV01_fwV01::checkProtocolValidity(string &message) {
    bool validFlag = true;
    message = "Valid protocol";
    switch (selectedProtocol) {
    case ProtocolConstant:
...
void MessageDispatcher_e16e_Artix7_PCBV01_fwV01::setFerdParameters() {
    unsigned int rangeCoeff;
    /*! At the moment the front end reset denoiser is only available for devices that apply the same current range on all channels */
    if (selectedCurrentRangesIdx[0] < CurrentRange200nA) {
        rangeCoeff = 4;
...
ErrorCodes_t MessageDispatcher_e16e_Artix7_PCBV01_fwV01::updateVoltageOffsetCompensations(vector <Measurement_t> &offsets) {
    for (int idx = 0; idx < currentChannelsNum; idx++) {
        offsets[idx] = voltageOffsetCompensationGain*(double)(infoStruct.offset[idx]);
    }
    return Success;
