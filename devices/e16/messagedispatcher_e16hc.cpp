...
MessageDispatcher_e16HC_PCBV01_EL06f_V01::MessageDispatcher_e16HC_PCBV01_EL06f_V01(string id) :
    ...
    selectedGpRangesIdx[GpChannelVoltageReference] = defaultGpRangesIdx[GpChannelVoltageReference];
    gpResolutions.resize(gpChannelsNum);
    gpOffsets.resize(gpChannelsNum);

    for (int idx = GpChannelVofs1; idx <= GpChannelVofs16; idx++) {
        gpRangesNum[idx] = 1;
        gpRangesArray[idx].resize(1);
        gpRangesArray[idx][0].step = 0.0625;
        gpRangesArray[idx][0].min = -2048.0;
        gpRangesArray[idx][0].max = gpRangesArray[idx][0].min+gpRangesArray[idx][0].step*USHORT_MAX;
        gpRangesArray[idx][0].prefix = UnitPfxMilli;
        gpRangesArray[idx][0].unit = "V";
        gpNames[idx] = "Voffset " + std::to_string(idx);
        defaultGpRangesIdx[idx] = 0;
        selectedGpRangesIdx[idx] = defaultGpRangesIdx[idx];
    }
    ...
    txStatus[txStatusIdx++] = 0x10; // CFG2
...
ErrorCodes_t MessageDispatcher_e16HC_PCBV01_EL06f_V01::setGpRange(uint16_t gpRangeIdx, uint16_t channelIdx, bool applyFlag) {
...
ErrorCodes_t MessageDispatcher_e16HC_PCBV01_EL06f_V01::getVoltageReferenceRanges(std::vector <RangedMeasurement_t> &ranges, uint16_t &defaultOption) {
    ranges.resize(1);
...
void MessageDispatcher_e16HC_PCBV01_EL06f_V01::initializeDevice() {
...
bool MessageDispatcher_e16HC_PCBV01_EL06f_V01::checkProtocolValidity(string &message) {
    bool validFlag = true;
...
ErrorCodes_t MessageDispatcher_e16HC_PCBV01_EL06f_V01::updateVoltageOffsetCompensations(vector <Measurement_t> &offsets) {
...
void MessageDispatcher_e16HC_PCBV01_EL06f_V01::updateVoltageReferenceOffsetCalibration() {
    /*! Voltage DAC Ext */
...
MessageDispatcher_e16HC_PCBV01_V01::MessageDispatcher_e16HC_PCBV01_V01(string id) :
    MessageDispatcher(id) {
...
ErrorCodes_t MessageDispatcher_e16HC_PCBV01_V01::setGpRange(uint16_t gpRangeIdx, uint16_t channelIdx, bool applyFlag) {
    if (channelIdx == 0) { /*! Channel 0 contains the voltage reference range */
        MessageDispatcher::setVoltageReferenceRange(gpRangeIdx, false);
    }
    return MessageDispatcher::setGpRange(gpRangeIdx, channelIdx, applyFlag);
...
ErrorCodes_t MessageDispatcher_e16HC_PCBV01_V01::getVoltageReferenceRanges(std::vector <RangedMeasurement_t> &ranges, uint16_t &defaultOption) {
    ranges.resize(1);
    ranges[VoltageReferenceRange2V] = voltageReferenceRangesArray[VoltageReferenceRange2V];
    defaultOption = VoltageReferenceRange2V;
    return Success;
...
void MessageDispatcher_e16HC_PCBV01_V01::initializeDevice() {
    this->updateVoltageReferenceOffsetCalibration();
    this->setGpRange(defaultGpRangesIdx[GpChannelVoltageReference], GpChannelVoltageReference, false);
    this->setSamplingRate(defaultSamplingRateIdx, false);
...
bool MessageDispatcher_e16HC_PCBV01_V01::checkProtocolValidity(string &message) {
    bool validFlag = true;
    message = "Valid protocol";
    switch (selectedProtocol) {
    case ProtocolConstant:
...
ErrorCodes_t MessageDispatcher_e16HC_PCBV01_V01::updateVoltageOffsetCompensations(vector <Measurement_t> &offsets) {
    for (int idx = 0; idx < currentChannelsNum; idx++) {
        offsets[idx] = voltageOffsetCompensationGain*(double)(infoStruct.offset[idx]);
    }
    return Success;
...
void MessageDispatcher_e16HC_PCBV01_V01::updateVoltageReferenceOffsetCalibration() {
    /*! Voltage DAC Ext */
    /*! \todo FCON serve davvero ricreare i coder? */
    DoubleCoder::CoderConfig_t doubleConfig;
    doubleConfig.initialByte = 86;
...
MessageDispatcher_e16HC_PCBV00_V03::MessageDispatcher_e16HC_PCBV00_V03(string id) :
    MessageDispatcher(id) {
...
void MessageDispatcher_e16HC_PCBV00_V03::initializeDevice() {
    this->updateVoltageReferenceOffsetCalibration();
    this->setSamplingRate(defaultSamplingRateIdx, false);
...
bool MessageDispatcher_e16HC_PCBV00_V03::checkProtocolValidity(string &message) {
    bool validFlag = true;
    message = "Valid protocol";
    switch (selectedProtocol) {
    case ProtocolConstant:
...
ErrorCodes_t MessageDispatcher_e16HC_PCBV00_V03::updateVoltageOffsetCompensations(vector <Measurement_t> &offsets) {
    for (int idx = 0; idx < currentChannelsNum; idx++) {
        offsets[idx] = voltageOffsetCompensationGain*(double)(infoStruct.offset[idx]);
    }
    return Success;
...
void MessageDispatcher_e16HC_PCBV00_V03::updateVoltageReferenceOffsetCalibration() {
    /*! Voltage DAC Ext */
    /*! \todo FCON serve davvero ricreare i coder? */
    DoubleCoder::CoderConfig_t doubleConfig;
    doubleConfig.initialByte = 86;
...
MessageDispatcher_e16HC_PCBV00_V02::MessageDispatcher_e16HC_PCBV00_V02(string id) :
    MessageDispatcher_e16HC_PCBV01_V01(id) {
...
ErrorCodes_t MessageDispatcher_e16HC_PCBV00_V02::setGpRange(uint16_t gpRangeIdx, uint16_t channelIdx, bool applyFlag) {
    return MessageDispatcher::setGpRange(gpRangeIdx, channelIdx, applyFlag);
}

ErrorCodes_t MessageDispatcher_e16HC_PCBV00_V02::getVoltageReferenceRanges(std::vector <RangedMeasurement_t> &ranges, uint16_t &defaultOption) {
    return MessageDispatcher::getVoltageReferenceRanges(ranges, defaultOption);
}

void MessageDispatcher_e16HC_PCBV00_V02::initializeDevice() {
    this->updateVoltageReferenceOffsetCalibration();
    this->setSamplingRate(defaultSamplingRateIdx, false);
...
MessageDispatcher_e16HC_PCBV00_V01::MessageDispatcher_e16HC_PCBV00_V01(string id) :
    MessageDispatcher_e16HC_PCBV00_V02(id) {
