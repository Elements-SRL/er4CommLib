...
MessageDispatcher_e4e_trigger_V01::MessageDispatcher_e4e_trigger_V01(string di) :
    MessageDispatcher(di) {
    ...
    ttlPulseTrainImplementedFlag = true;
    ...
    boolConfig.initialBit = 6;
    boolConfig.bitsNum = 1;
    ttlPulseTrainStartCoder = new BoolArrayCoder(boolConfig);
    doubleConfig.initialByte = 51;
    doubleConfig.initialBit = 0;
    doubleConfig.bitsNum = 12;
    doubleConfig.resolution = 0.1e-6;
    doubleConfig.minValue = 0.0;
    doubleConfig.maxValue = 200.0e-6;
    ttlPulseTrainDelayCoder = new DoubleTwosCompCoder(doubleConfig);
    doubleConfig.initialByte = 53;
    doubleConfig.initialBit = 0;
    doubleConfig.bitsNum = 12;
    doubleConfig.resolution = 0.1e-6;
    doubleConfig.minValue = 1.0-6;
    doubleConfig.maxValue = 200.0e-6;
    ttlPulseTrainDurationCoder = new DoubleTwosCompCoder(doubleConfig);
    boolConfig.initialByte = 55;
    boolConfig.initialBit = 0;
    boolConfig.bitsNum = 14;
    ttlPulseTrainPulsesNumberCoder = new BoolArrayCoder(boolConfig);
    doubleConfig.initialByte = 57;
    doubleConfig.initialBit = 0;
    doubleConfig.bitsNum = 25;
    doubleConfig.resolution = 0.1e-6;
    doubleConfig.minValue = 1.0-3;
    doubleConfig.maxValue = 1.0;
    ttlPulseTrainPeriodCoder = new DoubleTwosCompCoder(doubleConfig);
    ...
    txStatus[txStatusIdx++] = 0x00; // VInit
    txStatus[txStatusIdx++] = 0x00;
    txStatus[txStatusIdx++] = 0x00; // PDelay
    txStatus[txStatusIdx++] = 0x00;
    txStatus[txStatusIdx++] = 0x00; // PDuration
    txStatus[txStatusIdx++] = 0x00;
    txStatus[txStatusIdx++] = 0x00; // Npulses
    txStatus[txStatusIdx++] = 0x00;
    txStatus[txStatusIdx++] = 0x00; // PPeriod
    txStatus[txStatusIdx++] = 0x00;
    txStatus[txStatusIdx++] = 0x00;
    txStatus[txStatusIdx++] = 0x00;
}

MessageDispatcher_e4e_trigger_V01::~MessageDispatcher_e4e_trigger_V01() {

}

void MessageDispatcher_e4e_trigger_V01::initializeDevice() {
    this->setSamplingRate(defaultSamplingRateIdx, false);
...
bool MessageDispatcher_e4e_trigger_V01::checkProtocolValidity(string &message) {
    bool validFlag = true;
    message = "Valid protocol";
    switch (selectedProtocol) {
    case ProtocolConstant:
...
void MessageDispatcher_e4e_trigger_V01::setFerdParameters() {
    unsigned int rangeCoeff;
    /*! At the moment the front end reset denoiser is only available for devices that apply the same current range on all channels */
    if (selectedCurrentRangesIdx[0] < CurrentRange200nA) {
        rangeCoeff = 4;
...
ErrorCodes_t MessageDispatcher_e4e_trigger_V01::updateVoltageOffsetCompensations(vector <Measurement_t> &offsets) {
    for (int idx = 0; idx < currentChannelsNum; idx++) {
        offsets[idx] = voltageOffsetCompensationGain*(double)(infoStruct.offset[idx]);
    }
    return Success;
