...
MessageDispatcher_eNPR::MessageDispatcher_eNPR(string di) :
    ...
    oversamplingRatiosArray[OversamplingRatioX4] = 4;
    ...
    edhFormat =
            ...
            "Oversampling x%oversamplingRatio%: %oversamplingFlag%\n" // Oversampling x4: enabled
            "\n"
...
bool MessageDispatcher_eNPR::checkProtocolValidity(string &message) {
    bool validFlag = true;
...
void MessageDispatcher_eNPR::setFerdParameters() {
...
MessageDispatcher_eNPR_2Channels_V03::MessageDispatcher_eNPR_2Channels_V03(string di) :
    MessageDispatcher_eNPR_2Channels_V02(di) {
    ...
    defaultCurrentRangesIdx[1] = CurrentRange200pA;
    ...
    customDoublesNames[0] = "Duty cycle";
    customDoublesRanges.resize(customDoublesNum);
    customDoublesRanges[0].min = 0.0;
    customDoublesRanges[0].max = 100.0;
    customDoublesRanges[0].step = 100.0/1000.0;
    customDoublesRanges[0].prefix = UnitPfxNone;
    customDoublesRanges[0].unit = "%";
    customDoublesDefault.resize(customDoublesNum);
    customDoublesDefault[0] = 100.0;
    ...
    doubleConfig.maxValue = 100.0;
    doubleConfig.resolution = doubleConfig.maxValue/1000.0;
...
MessageDispatcher_eNPR_LegacyEdr3_V04::MessageDispatcher_eNPR_LegacyEdr3_V04(string di) :
    MessageDispatcherLegacyEdr3(di) {
...
bool MessageDispatcher_eNPR_LegacyEdr3_V04::checkProtocolValidity(string &message) {
    bool validFlag = true;
    message = "Valid protocol";
    string voltageLimit;
    switch (selectedVoltageRangeIdx) {
...
void MessageDispatcher_eNPR_LegacyEdr3_V04::setFerdParameters() {
    unsigned int rangeCoeff;
    /*! At the moment the front end reset denoiser is only available for devices that apply the same current range on all channels */
    if (selectedCurrentRangesIdx[0] < CurrentRange200nA) {
        rangeCoeff = 4;
...
MessageDispatcher_eNPR_LegacyEdr3_V03::MessageDispatcher_eNPR_LegacyEdr3_V03(string di) :
    MessageDispatcherLegacyEdr3(di) {
    ...
    protocolVoltageRanges[ProtocolVMax].min = voltageRangesArray[VoltageRange2V].min;
    protocolVoltageRanges[ProtocolVMax].max = voltageRangesArray[VoltageRange2V].max;
    ...
    protocolVoltageRanges[ProtocolVMin].min = voltageRangesArray[VoltageRange2V].min;
    protocolVoltageRanges[ProtocolVMin].max = voltageRangesArray[VoltageRange2V].max;
...
bool MessageDispatcher_eNPR_LegacyEdr3_V03::checkProtocolValidity(string &message) {
    bool validFlag = true;
    message = "Valid protocol";
    string voltageLimit;
    switch (selectedVoltageRangeIdx) {
    ...
    case ProtocolRamp:
        ...
        } else if (!(protocolVoltageRangesArray[selectedVoltageRangeIdx].includes(selectedProtocolVoltage[ProtocolVMax]))) {
            validFlag = false;
            message = "Vmax\nmust be within [-" + voltageLimit + "," + voltageLimit + "]mV";

        } else if (!(protocolVoltageRangesArray[selectedVoltageRangeIdx].includes(selectedProtocolVoltage[ProtocolVMin]))) {
            validFlag = false;
            message = "Vmin\nmust be within [-" + voltageLimit + "," + voltageLimit + "]mV";
...
void MessageDispatcher_eNPR_LegacyEdr3_V03::setFerdParameters() {
    unsigned int rangeCoeff;
    /*! At the moment the front end reset denoiser is only available for devices that apply the same current range on all channels */
    if (selectedCurrentRangesIdx[0] < CurrentRange200nA) {
        rangeCoeff = 4;
