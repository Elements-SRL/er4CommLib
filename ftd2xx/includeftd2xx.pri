...
else:msvc:win32:CONFIG(release, debug|release){
   contains(QT_ARCH, i386): LIBS += -L$$PWD/win/x86/ -lftd2xx
   else:contains(QT_ARCH, x86_64): LIBS += -L$$PWD/win/x64/ -lftd2xx
   }
else:msvc:win32:CONFIG(debug, debug|release){
contains(QT_ARCH, i386): LIBS += -L$$PWD/win/x86/ -lftd2xx
else:contains(QT_ARCH, x86_64): LIBS += -L$$PWD/win/x64/ -lftd2xx
}
